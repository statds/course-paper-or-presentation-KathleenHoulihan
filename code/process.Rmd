...
```{r Functions}
# Read the CSV file
data <- read.csv("data_2022_2023 (8).csv", header = TRUE)
...
#Add a "Mean minus (0.3) x Standard Deviation" column to the top_10_athletes table
  # Calculate the mean minus (0.3) x standard deviation
top_10_athletes <- top_10_athletes %>%
  mutate(Mean_minus_0.3xSD = Mean_Score - 0.3 * SD_Score)
...
#Calculate the means and standard deviations for the specified US female athletes across all apparatuses
 # Filter for USA female athletes
  ...
  target_last_names <- c("biles", "mcclain", "roberson", "sumanasekera", "jones", "carey", "chiles", "matthews", "wong")
